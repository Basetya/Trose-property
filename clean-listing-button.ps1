# =======================================================
# Script: clean-listing-button.ps1
# Deskripsi: Hapus tooltip petunjuk & bersihkan teks Listing Unit
# =======================================================
...
# 2. Hapus semua elemen floating widget lama (termasuk tooltip hitam di atasnya)
$Html = [System.Text.RegularExpressions.Regex]::Replace($Html, '<!-- FLOATING ACTIONS[\s\S]*?</div>\s*</div>', '', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
...
$Html = [System.Text.RegularExpressions.Regex]::Replace($Html, '<div class="floating-left-group"[\s\S]*?</div>\s*</div>', '', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
$Html = [System.Text.RegularExpressions.Regex]::Replace($Html, '<div class="floating-left-container"[\s\S]*?</div>\s*</div>', '', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)

# 3. Floating Widget Bersih (Hanya Tombol 'Listing Unit' + IG)
$CleanFloatingWidget = @"
    ...
    <div id="floating-left-widget" style="position: fixed; bottom: 24px; left: 24px; z-index: 99999; display: flex; flex-direction: column; align-items: flex-start; gap: 12px; font-family: system-ui, -apple-system, sans-serif;">
      ...
      <a href="https://www.rumah123.com/agen-properti/independent-property-agent/kusuma-properti-4373821/?utm_source=organic&amp;utm_medium=share-collection&amp;collectionUuid=5be8d605-387f-4aa5-ab99-bbb19af20cad#collection-Semua-Properti-5be8d605-387f-4aa5-ab99-bbb19af20cad" 
         ...
         style="display: inline-flex; align-items: center; gap: 8px; background: #0d47a1; color: #ffffff !important; font-size: 14px; font-weight: 600; padding: 10px 20px; border-radius: 50px; text-decoration: none; box-shadow: 0 4px 15px rgba(13,71,161,0.38); transition: transform 0.2s ease;">
...
Write-Host "-> Tooltip lama berhasil dihapus dan tombol Listing Unit sudah bersih!" -ForegroundColor Green

# 6. Commit & Push otomatis
git add .
git commit -m "clean: remove search hint tooltip and streamline Listing Unit button"
git push origin main

Write-Host "`nSELESAI! Perubahan sudah ter-push ke GitHub & Vercel." -ForegroundColor Cyan